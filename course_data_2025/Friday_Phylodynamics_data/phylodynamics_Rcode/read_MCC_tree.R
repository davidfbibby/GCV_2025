...
# 13 Oct 2023
# 8 Nov 2023 - latlon hpd regex fix
...
# 4 march 2019
# 8 Nov 2023 - adding minus sign to regex - thanks Michelle !
addLatLonHPD <- function(tr, regtype=3, latlonName="latlon",hpdNum=1) {
  ...
  #  #res1			<- gregexpr("latlon1_80\\%HPD_1=\\{.+\\}",tr$details)
  #  #res2			<- gregexpr("latlon2_80\\%HPD_1=\\{.+\\}",tr$details)
  #} else {
  #  res1			<- gregexpr("location1_80\\%HPD_1=\\{.+\\}",tr$details)
  #  res2			<- gregexpr("location2_80\\%HPD_1=\\{.+\\}",tr$details)
  #}
  rr1 <- paste0(latlonName,"1_80\\%HPD_",hpdNum,"=\\{.+\\}")
  rr2 <- paste0(latlonName,"2_80\\%HPD_",hpdNum,"=\\{.+\\}")
  res1<- gregexpr(rr1,tr$details)
  res2<- gregexpr(rr2,tr$details)
  
  pos1			<- unlist(res1)
  pos1_end              <- pos1+as.numeric(matrix(unlist(lapply(res1,attributes)),3,length(res1))[1,])
  inds1			<- which(pos1 > 0)
  ninds1		      <- which(pos1 < 0)
  latlon1_80		<- array(0, length(res1))
  latlon1_80[inds1] <- substring(tr$details[inds1], pos1[inds1]+18, pos1_end[inds1]-2)
  latlon1_80[ninds1]<- tr$latlon[ninds1,1]
  latlon1_80 <- apply(as.matrix(latlon1_80),1,getEl,ind=1,sep="\\}")
  ...
  pos2			<- unlist(res2)
  pos2_end              <- pos2+as.numeric(matrix(unlist(lapply(res2,attributes)),3,length(res2))[1,])
  inds2			<- which(pos2 > 0)
  ninds2		      <- which(pos2 < 0)
  latlon2_80		<- array(0, length(res2))
  latlon2_80[inds2] <- substring(tr$details[inds2], pos2[inds2]+18, pos2_end[inds2]-2)
  latlon2_80[ninds2]<- tr$latlon[ninds2,2]
  latlon2_80 <- apply(as.matrix(latlon2_80),1,getEl,ind=1,sep="\\}")
...
# find the discrete traits
  ...
  discreteRegex <- "[\\,\\&][A-Za-z0-9_\\-]+\\.prob" # to capture first trait - 13 Oct 2023 mod
...
read_latlon_mcc_tr <- function( trName, latlonName="latlon" ) {
	...
	trTxt <- readLines(trName)
	#trLine<- trTxt[ length(trTxt) - 1]
	trLine <- trTxt[grep("TREE",trTxt)]    # 13 Oct 2023
	trLine<- strsplit(trLine, "\\[\\&R\\] ")[[1]][2]
	pos1	<- gregexpr("\\[\\&",trLine)
	pos2  <- gregexpr("\\]",trLine)
	nnodes<- length(pos1[[1]])
